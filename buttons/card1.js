...
module.exports = {
	...
	execute: async (interaction, args, client) => {
		...
		let profileData = gp.get(`${interaction.guild.id}.${interaction.member.id}.profile`)
		...
		let statStr = interaction.message.components[1].components[4].customId

		let statArr = Array.from(statStr.split(','),Number);
		...
		if (interaction.message.embeds[1].image.url.includes('CardsTwo.jpg')) {
			console.log(2)
		}
		
		if (interaction.message.embeds[1].image.url.includes('CardsOne.jpg')) {
			const card1 = await Canvas.loadImage('./images/card0.png');
        ...
        const embedCard = new MessageEmbed()
            .setColor("2f3136")
            .setFooter({
            	text:`Fight started by ${interaction.memberTag} | Completed 0/1`,
            	iconURL: interaction.guild.iconURL()
            })
            .setImage('attachment://CardsOne.jpg')

		shuffled.splice(0, 1, '0');
