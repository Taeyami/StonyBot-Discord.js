...
module.exports = {
	...
	execute: (interaction, args, client) => {
		if (interaction.isButton() && interaction.customId == "coinsTop") {

        let dataCoins = cn.get(`${interaction.guild.id}`) || [0];
        ...
        const test = Object.keys(dataCoins).map(id => {
            return {
                userID: id,
                data: Object.values(dataCoins[id] || {}).reduce((a, b) => a + b, 0)
            }
        ...
        const embed = new MessageEmbed()
            ...
            .setDescription(`${topCoins} ⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯ \n\ **#${yourData[1]}.** <@${yourData[0].userID}>**:** ${yourData[0].data} Coins` )
            .setFooter({
